...
export const addContact = async(name, address, phone, email, dispatch) => {
   ...
   const response = await fetch('https://playground.4geeks.com/contact/agendas/yuanM/contacts', options)
   try {
    if (!response.ok) {
...
export const deleteContact = async(id, dispatch) => {
    ...
    const response = await fetch(`https://playground.4geeks.com/contact/agendas/yuanM/contacts/${id}`, options)
    try {
        if (!response.ok) {
